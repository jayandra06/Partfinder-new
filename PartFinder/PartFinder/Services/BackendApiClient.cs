...
{
    // Security: strict timeout, max response size 10 MB
    ...
    private async Task<(bool Ok, string? Error, T? Data)> GetAsync<T>(string path, CancellationToken ct)
    {
        ...
        {
            return (false, "Invalid organization context.", default);
        }
    ...
    // Security: only allow alphanumeric + hyphen/underscore in org codes (prevents header injection)
    private static bool IsValidOrgCode(string orgCode) =>
        !string.IsNullOrWhiteSpace(orgCode) &&
        orgCode.Length <= 64 &&
        orgCode.All(c => char.IsLetterOrDigit(c) || c == '-' || c == '_');
