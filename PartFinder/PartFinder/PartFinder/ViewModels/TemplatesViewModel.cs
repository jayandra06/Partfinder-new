...
public partial class TemplatesViewModel : ViewModelBase
{
    ...
    public PartTemplateDefinition? LeftCarouselTemplate => GetCarouselTemplate(offset: -1);

    public PartTemplateDefinition? RightCarouselTemplate => GetCarouselTemplate(offset: 1);

    public PartTemplateDefinition? LeftFarCarouselTemplate => GetCarouselTemplate(offset: -2);

    public PartTemplateDefinition? RightFarCarouselTemplate => GetCarouselTemplate(offset: 2);

    public bool ShowCarouselNeighbors => !IsCreatingTemplate && SelectedTemplate is not null && Templates.Count > 1;

    public bool ShowFarCarouselNeighbors => !IsCreatingTemplate && SelectedTemplate is not null && Templates.Count > 3;

    public string SelectedCarouselNumber => GetCarouselNumber(0);

    public string LeftCarouselNumber => GetCarouselNumber(-1);

    public string RightCarouselNumber => GetCarouselNumber(1);

    public string LeftFarCarouselNumber => GetCarouselNumber(-2);

    public string RightFarCarouselNumber => GetCarouselNumber(2);

    private PartTemplateDefinition? GetCarouselTemplate(int offset)
    {
        ...
        var selectedIndex = Templates.IndexOf(SelectedTemplate);
        if (selectedIndex < 0)
        {
            return null;
        }

        var wrappedIndex = (selectedIndex + offset + Templates.Count) % Templates.Count;
        return Templates[wrappedIndex];
    }

    private string GetCarouselNumber(int offset)
    {
        if (SelectedTemplate is null || Templates.Count == 0)
        {
            return "01";
        ...
        var wrappedIndex = (selectedIndex + offset + Templates.Count) % Templates.Count;
        return (wrappedIndex + 1).ToString("D2");
    }
    ...
    private void NotifyTemplateUiState()
    {
        ...
        OnPropertyChanged(nameof(LeftCarouselTemplate));
        OnPropertyChanged(nameof(RightCarouselTemplate));
        OnPropertyChanged(nameof(ShowCarouselNeighbors));
        OnPropertyChanged(nameof(LeftFarCarouselTemplate));
        OnPropertyChanged(nameof(RightFarCarouselTemplate));
        OnPropertyChanged(nameof(ShowFarCarouselNeighbors));
        OnPropertyChanged(nameof(SelectedCarouselNumber));
        OnPropertyChanged(nameof(LeftCarouselNumber));
        OnPropertyChanged(nameof(RightCarouselNumber));
        OnPropertyChanged(nameof(LeftFarCarouselNumber));
        OnPropertyChanged(nameof(RightFarCarouselNumber));
    }
